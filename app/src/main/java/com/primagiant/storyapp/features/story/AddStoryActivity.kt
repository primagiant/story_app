...
class AddStoryActivity : AppCompatActivity() {
    ...
    private fun addStory() {
        if (getFile != null) {
            ...
            val lat = validateLatitude(binding.inputLat.text.toString().toFloatOrNull())
            val lon = validateLongitude(binding.inputLon.text.toString().toFloatOrNull())
            ...
            Toast.makeText(
                this@AddStoryActivity, getString(R.string.add_picture_first), Toast.LENGTH_SHORT
            ).show()
        }
    ...
    private fun validateLatitude(lat: Float?): Float? {
        return if (lat == null) {
            null
        } else {
            if (lat >= -90 && lat <= 90) {
    ...
    private fun validateLongitude(lon: Float?): Float? {
        return if (lon == null) {
            null
        } else {
            if (lon >= -180 && lon <= 180) {
